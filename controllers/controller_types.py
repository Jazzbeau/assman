...
from typing import Awaitable, Callable, Any, Generic, TypeVar, Union, TypeAlias, List, Dict
...
class AppActivity(Generic[AppActivityType]):
    ...
    def to_dict(self) -> dict[str, JSONType]:
        ...
        if self.start_time:
            response['start_time'] = datetime.fromtimestamp(self.start_time, tz=timezone.utc).isoformat()
        else:
            response['start_time'] = None
        if self.end_time:
            response['end_time'] = datetime.fromtimestamp(self.end_time, tz=timezone.utc).isoformat()
        else:
...
HealthCheckT: TypeAlias = Union[
    CoreHealthCheck[AppHealthCheckType],
    ActivityHealthCheck[AppActivityType]
]
